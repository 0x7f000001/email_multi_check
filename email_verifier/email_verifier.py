...
def load_requests_config(requests_file: str, email: str) -> Tuple[str, dict, dict]:
    ...
    params = {k: v.format(email=email) for k, v in config["params"].items()} if "params" in config else {}
...
def verify_email_rcpt(email: str, sender_email: str, mx_servers: List[str], port: int) -> EmailVerificationResult:
    ...
    try:
        ...
        server = smtplib.SMTP_SSL(mx_record, port, timeout=15) if port == 465 else smtplib.SMTP(mx_record, port, timeout=15)
        ...
        status = code == 250
        message_text = (
            "Valid email" if code == 250 else
            "No such user!" if code == 550 else
            "Quota expired!" if code == 452 else
            f"SMTP error: {message.decode('utf-8') if isinstance(message, bytes) else message}"
        )
...
def verify_email_vrfy(email: str, sender_email: str, mx_servers: List[str], port: int) -> EmailVerificationResult:
...
def verify_email_expn(email: str, sender_email: str, mx_servers: List[str], port: int) -> EmailVerificationResult:
...
def verify_email_mail_from(email: str, sender_email: str, mx_servers: List[str], port: int) -> EmailVerificationResult:
    """Verify email using MAIL FROM / RCPT TO with RSET."""
    try:
        mx_record = mx_servers[0]
        logger.debug(f"Trying SMTP connection to {mx_record} on port {port} for MAIL FROM / RCPT TO")
        server = smtplib.SMTP_SSL(mx_record, port, timeout=15) if port == 465 else smtplib.SMTP(mx_record, port, timeout=15)
        server.set_debuglevel(0)
        server.ehlo()
        server.mail(sender_email)
        ...
        logger.debug(f"MAIL FROM / RCPT TO response on port {port}: code={code}, message={message}")
        status = code == 250
        message_text = (
            "Valid email" if code == 250 else
            "No such user!" if code == 550 else
...
def verify_email_web_auth(email: str) -> EmailVerificationResult:
    ...
    try:
        ...
        logger.debug(f"Sending web auth request for {email} to {url} with method {method}")
        ...
        return EmailVerificationResult(
            ...
            MX=[],
            method="web",
            web_auth_code=100 if not status else None,  # Assume code 100 for non-existing email
            data=data_text
        )
