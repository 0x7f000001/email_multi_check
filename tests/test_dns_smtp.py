...
def check_smtp_ports(email: str, ports: List[int], sender_email: str, mx_servers: List[str]) -> None:
    ...
    for port in ports:
        try:
            ...
            server = smtplib.SMTP_SSL(mx_record, port, timeout=10) if port == 465 else smtplib.SMTP(mx_record, port, timeout=10)
...
def check_email(email: str, sender_email: str, mode: str, ports: List[int]) -> None:
    ...
    if syntax_result:
        logger.error(f"Invalid email syntax: {syntax_result.message}, {syntax_result.data}")
        return

    if mode == "web" and not os.path.exists("url.cfg"):
        logger.error("Web auth mode requires url.cfg in the project root. "
    ...
    if mode == "all":
        ...
        for result in results:
            logger.info(f"Email verification ({result.method}): email={result.email}, message={result.message}, "
                        f"status={result.status}, MX={result.MX}, data={result.data}")
            print(f"{result}")
    else:
        if mode == "web":
            result = method_map[mode](email)
            logger.info(f"Email verification ({mode}): email={result.email}, message={result.message}, "
                        f"status={result.status}, MX={result.MX}, data={result.data}")
            print(f"{result}")
        else:
            for port in ports:
...
def parse_ports(port_str: str) -> List[int]:
    ...
    try:
        ...
        logger.error(f"Invalid port format: {port_str}. Expected comma-separated integers.")
        raise
